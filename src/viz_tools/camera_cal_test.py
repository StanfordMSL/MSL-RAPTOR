...
class camera_cal_test_node:
    def __init__(self):
        ...
        if False:
            ...
            # new candidate:
            self.dist_coefs = np.array([-0.43865152,  0.24289838,  0.00008457,  0.00023874,  0.0764542 ])
            self.K = np.array([[484.87367662,   0.        , 328.8659078 ],
                                [  0.        , 485.00833725, 248.7795412 ],
                                [  0.        ,   0.        ,   1.        ]])
            self.R_cam_ego = np.reshape([ 0.02435766, -0.99953732, -0.01821654, -0.03712431,  0.01730499, -0.99916081,  0.99901376,  0.02501349, -0.03668562], (3,3))
            self.t_cam_ego = np.asarray([ 0.00666853,  0.01268759, -0.05903087])
